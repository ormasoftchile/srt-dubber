...
**Recommendation for Steven:** Apply same pattern to Review screen (play/redo/next/back/export/quit). Very similar interaction model; should reuse FlowEffects pattern and add a separate `src/core/review_flow.{hpp,cpp}` with its own state + command + effects structs. Enables deterministic test harness for multi-screen flows.
