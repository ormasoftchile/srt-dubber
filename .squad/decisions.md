...
### Recording Flow State Machine Pattern
**Author:** Alan  
**Date:** 2025-01-29  
**Status:** Implemented
...
**Pure state machine:** `recording_step(state, cmd) → (new_state, effects)` enables deterministic testing without TUI or audio.

**Turn-based harness:** `tools/recording_harness.cpp` drives same logic via JSON stdin/stdout protocol — enables scripted interaction testing.

**Test suite:** `tests/recording_flow_test.cpp` — 22 tests covering all state transitions (100% pass rate).

**TUI integration:** `src/tui/screens/recording_screen.cpp` reduced from ~270 to ~230 lines; key handling delegated to state machine; TUI becomes thin adapter applying effects.

**Build change:** Added `srt_dubber_core` library (core + srt parser, no audio/TUI deps) for test/harness support.

**Limitations:** Countdown timer thread remains in TUI (timing is display concern); harness uses synthetic CountdownComplete to simulate timer completion.
